...
function createSocketIOServer(server) {
  ...
  io.on('connection', function(socket) {
    ...
    socket.on('ICE-Candidate', function(data) {
      ...
      var newHandle = sockIDtoInfo[socket.id].handle === 'bob' ? 'bill' : 'bob';
      io.sockets.connected[handleToSockID[newHandle]].emit('ICE-Candidate', {
        //handle: newHandle,
    ...
    socket.on('SDP-Offer', function(data) {
      ...
      io.sockets.connected[handleToSockID[newHandle]].emit('SDP-Offer', {
        //handle: newHandle,
        desc: data.desc
      });
    });
    ...
    socket.on('SDP-Answer', function(data) {
      var newHandle = sockIDtoInfo[socket.id].handle === 'bob' ? 'bill' : 'bob';
      io.sockets.connected[handleToSockID[newHandle]].emit('SDP-Answer', {
        //handle: newHandle,
        desc: data.desc
      });
    });
