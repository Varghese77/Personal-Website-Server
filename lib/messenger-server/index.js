...
Room.prototype.addUser = function (handle) {
  if (!this.users[handle]) {
    this.users[handle] = true;
    this.size++;
    return true;
  }
  return false;
};
Room.prototype.removeUser = function (handle) {
  if (this.users[handle]) {
...
function executeCommand(socket, data) {
  ...
  switch (true) {
    ...
    case command === "/getuserinfo":
      executeGetUserInfoCommand(socket);
      return;
    case command === "/listroompeople":
      executeListRoomPeopleCommand(socket, parts);
      return;
    case command === '/crashserver':
	      // This command is 'special' and crashes the server to test
        // exception handling
...
function executeCreateRoomCommand(socket, parts) {
  if (parts.size != 2 || rooms[parts[1]]) {
    sendError(socket, "Couldn't create room, either " + 
      "command wasn't used correctly or the room name already exists");
      return;
  }
  ...
  rooms[parts[1]] = true;
  socket.emit("console-message", {
    message: "Room \"" + parts[1] + "\" created!"
  });
}

function executeJoinCommand(socket, parts) {
  if (parts.size != 2 || !rooms[parts[1]]) {
    sendError(socket, "Couldn't join room, either " + 
      "command wasn't used correctly or the room name doesn't");
      return;
  }
  ...
  socket.join(parts[1]);
  socket.emit("console-message", {
    message: "Room \"" + parts[1] + "\" joined!"
  });
}

function executeGetUserInfoCommand(socket) {
var msg = 'Your current handle is \"' + sockIDtoInfo[socket.id].handle + 
  '\" and you are currently in room \"' + sockIDtoInfo[socket.id].room + '\".'
...
function executeListRoomPeopleCommand(socket, parts) {
  if (parts.size != 2 || !rooms[parts[1]]) {
    sendError(socket, "Couldn't join room, either " + 
      "command wasn't used correctly or the room doesn't exist");
      return;
  }
  socket.emit("console-message", {
    message: "People currently in room \"" + parts[1] + "\"..."
  });
  socket.emit("list", roomMetaInfo[parts[1]].users);
}
...
function createSocketIOServer(server) {
  ...
  io.on("connection", function (socket) {
    ...
    console.log(defaultHandle + " connected");
    pageViews++;

    // Handle Socket Input
    socket.on("command", function (data) {
      console.log(sockIDtoInfo[socket.id].handle + ' entered command "' + data.message + '"');
      executeCommand(socket, data);
    });

    socket.on("disconnect", function (data) {
      console.log(sockIDtoInfo[socket.id].handle + " Disconnected");
      var room = sockIDtoInfo[socket.id].room;
      roomMetaInfo[room].removeUser(sockIDtoInfo[socket.id].handle);
    ...
    socket.on("chat", function (data) {
      if (data.message && data.message.charAt(0) != "/") {
        console.log(sockIDtoInfo[socket.id].handle + ' Says "' + data.message + '"');
